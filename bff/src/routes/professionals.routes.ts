...
// Proxy para todas las rutas de profesionales
router.all('*', asyncHandler(async (req: Request, res: Response) => {
  const services = (req as any).services;
  const targetUrl = `${services.PROFESSIONALS}/api/v1/professionals${req.path}`;
  ...
  try {
    ...
    console.error(`[BFF] Error proxying to professionals service:`, error.message);
    ...
    if (error.response) {
      ...
      res.status(503).json({
        success: false,
        message: 'Servicio de profesionales no disponible',
