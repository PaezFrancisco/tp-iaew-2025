...
// Proxy para todas las rutas de turnos
router.all('*', asyncHandler(async (req: Request, res: Response) => {
  const services = (req as any).services;
  const targetUrl = `${services.APPOINTMENTS}/api/v1/appointments${req.path}`;
  ...
  try {
    ...
    console.error(`[BFF] Error proxying to appointments service:`, error.message);
    ...
    if (error.response) {
      ...
      res.status(503).json({
        success: false,
        message: 'Servicio de turnos no disponible',
